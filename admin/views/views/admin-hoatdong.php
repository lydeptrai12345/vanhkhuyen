				...
				<div class="card-body p-0 pb-3 text-center">
					<table class="table mb-0">
						...
						<?php
	                        ...
	                        else
	                        {
	                            ...
	                            $query_pg="SELECT COUNT(id) FROM hoatdong";
							...
							$query = "SELECT * FROM hoatdong ORDER BY id DESC LIMIT {$start},{$limit}";
						...
						<tbody>
							<tr>
								...
								<td class="text-justify"><?php echo $item['mo_ta'] ?></td>
								<td>
									<?php
								...
								<td>
									<a href="admin-hoatdong-sua.php?id=<?php echo $item['id']; ?>"><i class="material-icons action-icon">edit</i></a>
								</td>
								<td>
									<a href="admin-hoatdong-xoa.php?id=<?php echo $item['id']; ?>" onclick = "return confirm('Bạn có muốn xoá <?php echo $item['tieude'] ?>')" ><i class="material-icons action-icon">delete_outline</i></a>
								</td>
							</tr>
						</tbody>
						<?php } ?>
					...
					echo "<nav aria-label='Page navigation example'>";
		                ...
		                if($per_page > 1)
		                {
		                    ...
		                    if($current_page !=1)
		                    {
		                        echo "<li class='page-item' class='float-left'><a class='page-link' href='admin-hoatdong.php?s=".($start - $limit)."&p={$per_page}'>Trở về</a></li>";
		                    }
		                    ...
		                    { 
		                        if($i != $current_page)
		                        {
		                            echo "<li class='page-item'><a class='page-link' href='admin-hoatdong.php?s=".($limit *($i - 1))."&p={$per_page}'>{$i}</a></li>";
		                        }
		                        else
		                        {
		                    ...
		                    {
		                        echo "<li class='page-item' ><a class='page-link' href='admin-hoatdong.php?s=".($start + $limit)."&p={$per_page}'>Tiếp</a></li>";  
		                    }
		                }
		                echo "</ul>";
		            echo "</nav>"          
