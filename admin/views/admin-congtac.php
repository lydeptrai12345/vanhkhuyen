				...
				<div class="card-body p-0 pb-3 text-center">
					<table class="table mb-0">
						...
						<thead class="bg-light">
							<tr>
								...
								<th scope="col" class="border-0">Công việc</th>
						...
						<?php
	                        ...
	                        else
	                        {
	                            ...
	                            $query_pg="SELECT COUNT(cong_tac_id) FROM congtac WHERE nhan_vien_id = $id";
							...
							$query = "SELECT * FROM congtac WHERE nhan_vien_id = $id ORDER BY cong_tac_id DESC, ngay_ket_thuc DESC LIMIT {$start},{$limit}";
						...
						<tbody>
							<tr>
								...
								<td><?php echo date( "d/m/Y", strtotime($item['ngay_bat_dau'])) ?></td>
								<td><?php echo date( "d/m/Y", strtotime($item['ngay_ket_thuc'])) ?></td>
								<td><?php echo $item['cong_viec'] ?></td>
								<td><?php echo $item['ghi_chu'] ?></td>
								<?php
								//Lay ngay ket thuc co trong bang hop dong de so sanh
			                        $query_id="SELECT ngay_ket_thuc FROM congtac WHERE nhan_vien_id = $id ORDER BY ngay_ket_thuc DESC LIMIT 0,1";
			                        $result_id=mysqli_query($dbc,$query_id);
			                        if(mysqli_num_rows($result_id)==1)
			                        {
			                        ...
			                        if($item['ngay_ket_thuc'] < $ngayktln)
			                        {
			                        ?>
			                        	<td style="color : blue">Đã công tác</td>
			                    	<?php
			                        }
			                        else
			                        {  
									?>
									<td>
										<a href="admin-congtac-sua.php?id=<?php echo $item['cong_tac_id']; ?>"><i class="material-icons action-icon">edit</i></a>
									</td>
									<?php
									}
								?>
								<td>
									<a href="admin-congtac-xoa.php?idnv=<?php echo $id ?>&id=<?php echo $item['cong_tac_id']; ?>" onclick="return confirm('Bạn có thực sự muốn xóa <?php echo $item['ten']; ?>');"><i class="material-icons action-icon">delete_outline</i></a>
								</td>
							</tr>
						</tbody>
						<?php } ?>
					...
					echo "<nav aria-label='Page navigation example'>";
		                ...
		                if($per_page > 1)
		                {
		                    ...
		                    if($current_page !=1)
		                    {
		                        echo "<li class='page-item' class='float-left'><a class='page-link' href='admin-congtac.php?id=".$id."&s=".($start - $limit)."&p={$per_page}'>Trở về</a></li>";
		                    }
		                    ...
		                    { 
		                        if($i != $current_page)
		                        {
		                            echo "<li class='page-item'><a class='page-link' href='admin-congtac.php?id=".$id."&s=".($limit *($i - 1))."&p={$per_page}'>{$i}</a></li>";
		                        }
		                        else
		                        {
		                    ...
		                    {
		                        echo "<li class='page-item' ><a class='page-link' href='admin-congtac.php?id=".$id."&s=".($start + $limit)."&p={$per_page}'>Tiếp</a></li>";  
		                    }
		                }
		                echo "</ul>";
		            echo "</nav>"          
